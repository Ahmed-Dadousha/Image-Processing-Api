...
exports.images.get('/', (req, res) =>
  __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (height <= 0 || width <= 0) {
      res
        .status(400)
        .send('Please make sure width and height are biger than 0');
      return;
    }
    ...
    if (/d/.test(filename)) {
      res
        .status(400)
        .send('Please make sure filename Contains letters only ' + filename);
      return;
    }
    //Get image path in full folder
    const fullImagePath = path_1.default.resolve(
      __dirname,
      `../../../assets/images/full/${filename}.jpg`
    );
    // thumb path in the ${filename}-${height}x${width} format to save different dimensions
    const thumbImagePath = `${path_1.default.resolve(
      __dirname,
    ...
    if (fs_1.default.existsSync(fullImagePath) === true) {
      ...
      if (fs_1.default.existsSync(thumbImagePath) === true) {
        ...
        (0, imageProcessing_1.resizeImage)(
          fullImagePath,
          filename,
          width,
          height
